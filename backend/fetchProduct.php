...
if ($status == "items") {
    ...
    $stm = $db->prepare("SELECT * FROM product WHERE product LIKE :key OR location LIKE :key OR model LIKE :key OR item LIKE :key OR price LIKE :key OR dep LIKE :key OR employee LIKE :key");
}
$no = 0;
...
?>
    <tr>
        ...
        <td>
            <button type="button" class="btn btn-danger cause" style="display:;" data-bs-toggle="modal" data-bs-target="#basicModal" onclick="viewProductImage('<?php echo $pimage ?>')" style="background: transparent;">
